...
class CoreModel(models.Model):
  ...
  def fit(self,
    ...
    It is recommended to used a Pandas Dataframe dataset, and to convert it to
    a TensorFlow dataset with "dataframe_to_tf_dataset":
    ...
    IMPORTANT: This model on the entire dataset at once. This has the following
    consequences:
    ...
    In this case, the dataset is ready asynchronous in between the workers. The
    ...
    Here is a full examples of distributed training:
      ...
      def dataset_fn(context, paths, training=True):
        ...
        # Split the dataset example-wize among the workers.
    ...
    Args:
      ...
      validation_steps: Number of steps in the evaluation dataset when
        ...
        yes supported for distributed datasets).
    ...
    if steps_per_epoch is not None:
      tf_logging.warning(
          "The steps_per_epoch argument is not deprecated. Instead, feed a "
  ...
  def _fit_implementation(
    ...
    This method performs operations that similars as the Keras' fit function.
    ...
    Details:
      ...
      3. Train the Yggdrasil model. Depending on learner, might also evaluate
        the model (on the validation dataset or with out-of-bag).
    ...
    # missconfigured, the error is raised immediately (instead of after
    # training)..
    ...
    if distribution_config is None:
      ...
      data_handler = get_data_handler(
          x=x, y=y, sample_weight=sample_weight, model=self)
      ...
      if not self.capabilities().support_partial_cache_dataset_format:
        raise ValueError(
            ...
            "DistributedGradientBoostedTreesModel nstead of "
            "GradientBoostedTreesModel.")

      # Make the workers read the entire dataset.
      # If a worker is interrupted, it restart reading its part of the dataset
      # from the start.
